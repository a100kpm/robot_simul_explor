function [xd,yd,ppoids1] = choix(Carte_decision,Carte_nav,Carte,position,position_depart,graph,col,row,suppr)
...
%% éventuellement très loin après, optimiser pour observer les points a 1(non) ou 3(oui) voisins
% %  risque d'arrêt prématuré
BBB=Carte_decision(:,:,2); %%90-94
[col2,row2]=find(BBB==2);
...
for i=1:size(c,1)
%     if Carte_nav(c(i,1),c(i,2),2)==255 %% test sans le if
